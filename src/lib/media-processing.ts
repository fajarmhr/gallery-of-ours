...
/** Builds web-sized versions, resolves the place and asks AI for a caption. Runs after the upload response. */
export async function processMedia(mediaId: string, options: { skipAI?: boolean } = {}) {
  ...
  try {
    ...
    await db.update(media).set({ variants, width, height, placeId, status: "ready" }).where(eq(media.id, item.id));
    await db
      .update(albums)
      .set({ coverMediaId: item.id })
      .where(and(eq(albums.id, item.albumId), isNull(albums.coverMediaId)));

    if (!options.skipAI && variants.medium) {
      try {
        const result = await captionImage(await files.getBuffer(variants.medium.key));
        if (result) {
